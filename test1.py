...
class PressureSensorApp(tk.Tk):
    ...
    def build_frm_heatmap_colorbar(self, parent: ttk.Frame):
        # Ensure the program will not be too large for the screen
        colorbar_width = min(self.app_state.heatmap_canvas_size[0], self.winfo_screenwidth()*0.10)
        colorbar_height = min(self.app_state.heatmap_canvas_size[1], self.winfo_screenheight()*0.7)
        
        # Create the canvas for the colorbar
        self.canvas_colorbar = tk.Canvas(
            parent, bg="white", highlightthickness=0, border=0, relief="flat", 
            width=colorbar_width, height=colorbar_height
        )
        self.canvas_colorbar.grid(row=self.winfo_screenheight(), column=self.winfo_screenwidth(), sticky="nsew")
        self.canvas_colorbar.bind("<Configure>", self.on_heatmap_resize)
        parent.columnconfigure(0, weight=100)
        parent.rowconfigure(0, weight=100)
    ...
    def get_data_from_com_port(self) -> np.ndarray:
        ...
        if not self.serialcomm.is_open:
            ...
            if not opened:
                ...
                #print("Failed to open port")  # DEBUGGING LINE
                self.new_state.com_port = None
                self.app_state.com_port = None
        ...
        #print("Succeeded to open port")  # DEBUGGING LINE
        self.serialcomm.write(b'\x01')

        # Get the raw data from the sensor
        #raw_data: np.ndarray = self.serial_read_int_array() # voltages from arduino
        # TODO: calculations on the raw data, calibrations, etc...
        #const_resistor = 220
        cols = 16
        rows = 16
        # resistor_data = np.zeros((rows, cols)) # 16 rows and collumns
        # r = np.zeros(rows)
        # voltage_sent = 5 # voltage sent out by arduino through the mat
        # voltage_out = np.zeros((rows, cols)) # coming out of each collumn
        # voltage_divider = np.zeros((rows, cols+1)) # resistances at nodes. The last index in col is the sum of all node resistances
        # To calculate to resistance: ((float)raw_data[i][j] * (5.0-volts/1023.0 - bites)) / 0.015Amps
        # for i in range (0, rows):
        #     r[i] = 1
        #     for j in range(0, cols):
        #         resistor_data[i][j]=(float(raw_data[i][j]) * (5.0/1023.0)) / 0.015
        #         r[i] *= resistor_data[i][j]
        # # calculate resistances from V-dividers of each column and create the sum of them for the whole row 
        # for i in range (0, rows):
        #     for j in range(1, cols):
        #         voltage_divider[i][j] = resistor_data[i][j] * resistor_data[i][j-1] * const_resistor
        #         voltage_divider[i][cols] += voltage_divider[i][j]
        # # calculate voltage out for each column that gets the voltage
        # for i in range (0, rows): # fixme: it is suppose to calculate the row and column that is being sent the voltage but it just does all of them at once rn
        #     for j in range(1, cols):
        #         voltage_out[i][j] = (voltage_sent * voltage_divider[i][j])/(r[i]+voltage_divider[i][cols])
        # print(voltage_out, "\n")
        # #set max and min
        # MAX_VAL = np.max(voltage_out)
        # MIN_VAL = np.min(voltage_out)
        # print("Max :", MAX_VAL, "  Min: ", MIN_VAL, "\n")
        # simulate a rango of data
        for i in range(0, rows):
            for j in range(0, cols):
                voltage_out = random.randrange(0, 5, .5)
        return voltage_out
    ...
    def draw_heatmap(self, canvas_resized: bool = False):
        ...
        # Generate a vertical gradient from MIN_VAL to MAX_VAL
        colorbar_height = int(min(self.app_state.heatmap_canvas_size[1], self.winfo_screenheight()*0.75))
        colorbar_width = int(min(self.app_state.heatmap_canvas_size[1], self.winfo_screenwidth()*0.03))
        gradient = np.linspace(MIN_VAL, MAX_VAL, colorbar_height).reshape(colorbar_height, 1)
        #repeat to increase bar thickness
        # Repeat it to make the color bar thicker
        gradient = np.repeat(gradient, colorbar_width, axis=1)
        # Normalize the gradient (just like the heatmap)
        norm = Normalize(vmin=MIN_VAL, vmax=MAX_VAL)
        normalized_gradient = norm(gradient)

        # Apply the Inferno colormap
        colormap = cm.get_cmap("inferno")
        colorbar_image = colormap(normalized_gradient)[:, :, :3]  # Remove alpha channel
        colorbar_image = (colorbar_image * 255).astype(np.uint8)  # Convert to RGB (0-255)

        # Convert NumPy array to an image
        img = Image.fromarray(colorbar_image)
        self.colorbar_photo = ImageTk.PhotoImage(img)

        # Display the image in the canvas
        self.canvas_colorbar.create_image(0, 0, anchor="nw", image=self.colorbar_photo)
